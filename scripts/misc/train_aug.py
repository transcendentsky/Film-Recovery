...
output_dir = tdir("output/train", "aug"+generate_name())
...
def main():
    ...
    if True:
        ...
        pretrained_dict = torch.load("/home1/quanquan/code/film_code/output/train/aug20201129-210822-VktsHX/cmap_aug_19.pkl", map_location=None)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_idx, data in enumerate(dataset_train_loader):
            ...
            optimizer.step()
            scheduler.step()
            print("\r Epoch[{}/{}] \t batch:{} \t \t loss: {}".format(epoch, args.epochs, batch_idx, loss.item()), end=" ") 
            
            lr = get_lr(optimizer)
            # w("check code")
            # break
        
        writer_tb((loss, loss_ab, loss_uv, loss_cmap, lr), epoch=epoch)
...
def writer_tb(loss_tuple, epoch):
    ...
    writer.add_scalar('summary/train_loss',        loss.item(),      global_step=epoch)
    writer.add_scalar('summary/train_cmap_loss',   loss_cmap.item(), global_step=epoch)
    writer.add_scalar('summary/train_uv_loss',     loss_uv.item(),   global_step=epoch)
    # writer.add_scalar('summary/train_normal_loss', loss_nor.item(),  global_step=epoch)
    # writer.add_scalar('summary/train_depth_loss',  loss_dep.item(),  global_step=epoch)
    writer.add_scalar('summary/train_ab_loss',     loss_ab.item(),   global_step=epoch)
    # writer.add_scalar('summary/train_back_loss',   loss_bg.item(),   global_step=epoch)
