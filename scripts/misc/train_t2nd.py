...
def train(args, model, device, train_loader, optimizer, criterion, epoch, writer, output_dir, isWriteImage, isVal=False,
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        dep_map_gt, nor_map_gt, threeD_map_gt = dep_map_gt.to(device), nor_map_gt.to(device), threeD_map_gt.to(device)
        optimizer.zero_grad()
        nor_map, dep_map = model(threeD_map_gt)
        loss_nor = criterion(nor_map, nor_map_gt).float()
        loss_dep = criterion(dep_map, dep_map_gt).float()
        loss = loss_nor + loss_dep
        loss.backward()
        optimizer.step()
        lr = get_lr(optimizer)
        if batch_idx % args.log_intervals == 0:
            print(
                'Epoch:{} \n batch index:{}/{}||lr:{:.8f}||loss:{:.6f}||loss normal:{:.6f}||loss depth:{:.6f}'.format(epoch, batch_idx + 1,
                                                                                           len(
                                                                                               train_loader.dataset) // args.batch_size,
                                                                                           lr, loss.item(), loss_nor.item(), loss_dep.item()))
        ...
        if isWriteImage:
            if batch_idx == (len(train_loader.dataset) // args.batch_size)-1:
                ...
                for k in range(5):
                    ...
                    output_nor_pred = output_dir1 + 'pred_nor_ind_{}'.format(k) + '.jpg'
                    output_nor_gt = output_dir1 + 'gt_nor_ind_{}'.format(k) + '.jpg'
                    output_depth_pred = output_dir1 + 'pred_depth_ind_{}'.format(k) + '.jpg'
                    output_depth_gt = output_dir1 + 'gt_depth_ind_{}'.format(k) + '.jpg'
                    write_image_tensor(nor_map[k, :, :, :], output_nor_pred, 'gauss', mean=[0.584, 0.294, 0.300],
                                           std=[0.483, 0.251, 0.256])
                    write_image_tensor(nor_map_gt[k, :, :, :], output_nor_gt, 'gauss', mean=[0.584, 0.294, 0.300],
                                           std=[0.483, 0.251, 0.256])
...
def test(args, model, device, test_loader, criterion, epoch, writer, output_dir, isWriteImage, sstep):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            nor_map_gt = data[3]
            threeD_map_gt = data[4]
            dep_map_gt, nor_map_gt, threeD_map_gt = dep_map_gt.to(device), nor_map_gt.to(device), threeD_map_gt.to(
                device)
            nor_map, dep_map = model(threeD_map_gt)
            loss_nor = criterion(nor_map, nor_map_gt).float()
            loss_dep = criterion(dep_map, dep_map_gt).float()
            test_loss = loss_nor + loss_dep
            loss_sum = loss_sum + test_loss
            test_nor_sum = test_nor_sum + loss_nor
            test_dep_sum = test_dep_sum + loss_dep
            if args.calculate_CC:
                c_nor = metrics.calculate_CC_metrics(nor_map, nor_map_gt)
    ...
    test_loss_nor = test_nor_sum / (len(test_loader.dataset) / args.test_batch_size)
    test_loss_dep = test_dep_sum / (len(test_loader.dataset) / args.test_batch_size)
    print(
        'Epoch:{} \n batch index:{}/{}||loss:{:.6f}||loss normal:{:.6f}||loss depth:{:.6f}'.format(epoch, batch_idx + 1,
                                                                   len(test_loader.dataset) // args.batch_size,
                                                                   test_loss.item(), test_loss_nor.item(), test_loss_dep.item()))
    if args.calculate_CC:
        cc_nor = cc_nor / (len(test_loader.dataset) / args.test_batch_size)
        cc_dep = cc_dep / (len(test_loader.dataset) / args.test_batch_size)
    if args.calculate_CC:
        print('CC_nor: {}\t CC_dep: {}\t'.format(cc_nor,cc_dep))

    if args.write_txt:
        txt_dir = 'output_txt/' + model_name + '.txt'
        f = open(txt_dir, 'a')
        f.write(
            'Epoch: {} \t Test Loss: {:.6f}\t Normal Loss: {:.6f}\t Depth Loss: {:.6f}\t Nor CC: {:.6f}\t Depth CC: {:.6f}\n'.format(
                epoch, test_loss.item(), test_loss_nor.item(), test_loss_dep.item(),cc_nor, cc_dep))
        f.close()
    if args.write_summary:
        print('sstep', sstep)
...
def main():
    # Model Build
    model = modules.ThreeD2NorDepth(downsample=3)
    ...
    if True:
        ...
        pretrained_dict = torch.load('model/t2nd_model/t2nd_35.pkl', map_location=None)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if isTrain:
            ...
            break
        scheduler.step()
        t2d_save_model = 'model/t2nd_model/'
