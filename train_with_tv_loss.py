...
import metrics
import numpy as np
from write_image_mask import *
from tensorboardX import SummaryWriter
from tv_losses import TVLoss

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "6, 0, 7"
...
def train(args, model, device, train_loader, optimizer, criterion, epoch, writer, output_dir, isWriteImage, isVal=False,
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        b, c, h, w = nor_map_gt.size()
        nor_map_gt = nor_map_gt.mul(mask_map_gt.expand(b,3,h,w))
        threeD_map_gt = threeD_map_gt.mul(mask_map_gt.expand(b,3,h,w))
        dep_map_gt = dep_map_gt.mul(mask_map_gt.expand(b,1,h,w))
        optimizer.zero_grad()
        ...
        loss_alb = criterion(alb_map, alb_map_gt).float()
        ...
        loss = 4 * loss_uv + 4 * loss_alb + loss_nor + loss_dep + 2 * loss_mask + loss_threeD + \
               cons_threeD2nor + cons_threeD2dep + cons_dep2nor + cons_nor2dep + 0.5*loss_tv
        loss.backward()
        optimizer.step()
        lr = get_lr(optimizer)
        ...
        if isWriteImage:
            if batch_idx == (len(train_loader.dataset) // args.batch_size)-1:
                ...
                for k in range(5):
                    ...
                    """pred"""
                    write_image_tensor(uv_pred, output_uv_pred, 'std', device=device,mask=back_pred)
                    write_image_tensor(back_pred, output_back_pred, '01')
                    write_image_tensor(albedo_pred, output_ab_pred, 'std')
                    write_image_tensor(cmap_pred, output_3d_pred, 'gauss', mean=[0.1109, 0.3162, 0.2862],std=[0.1101, 0.3288, 0.2993],mask=back_pred)
                    write_image_tensor(depth_pred, output_depth_pred, 'gauss', mean=[0.3086], std=[0.3101],mask=back_pred)
                    write_image_tensor(normal_pred, output_normal_pred, 'gauss', mean=[0.5623, 0.2883, 0.2918],std=[0.4808, 0.2616, 0.2623],mask=back_pred)
                    write_image_np(bw_pred, output_bw_pred)
                    """gt"""
                    write_image_tensor(ori_gt, output_ori, 'std',mask=mask_gt)
                    write_image_tensor(uv_gt, output_uv_gt, 'std', device=device,mask=mask_gt)
                    write_image_tensor(mask_gt, output_back_gt, '01',mask=mask_gt)
                    write_image_tensor(ab_gt, output_ab_gt, 'std',mask=mask_gt)
                    write_image_tensor(cmap_gt, output_cmap_gt, 'gauss', mean=[0.1109, 0.3162, 0.2862],std=[0.1101, 0.3288, 0.2993],mask=mask_gt)
                    write_image_tensor(depth_gt, output_depth_gt, 'gauss', mean=[0.3086], std=[0.3101],mask=mask_gt)
                    write_image_tensor(normal_gt, output_normal_gt, 'gauss', mean=[0.5623, 0.2883, 0.2918],
                                           std=[0.4808, 0.2616, 0.2623],mask=mask_gt)
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            if args.calculate_CC:
                c_alb = metrics.calculate_CC_metrics(alb_map, torch.unsqueeze(alb_map_gt, 1))
                ...
                c_dewarp_alb = metrics.calculate_CC_metrics(dewarp_ab, dewarp_ab_gt)
            ...
            if isWriteImage:
                if batch_idx == (len(test_loader.dataset) // args.test_batch_size)-1:
                    ...
                    for k in range(args.test_batch_size):
                        ...
                        """pred"""
                        write_image_tensor(uv_pred, output_uv_pred, 'std', device=device,mask=back_pred)
                        write_image_tensor(back_pred, output_back_pred, '01',mask=back_pred)
                        write_image_tensor(albedo_pred, output_ab_pred, 'std',mask=back_pred)
                        write_image_tensor(cmap_pred, output_3d_pred, 'gauss', mean=[0.1109, 0.3162, 0.2862],
                                           std=[0.1101, 0.3288, 0.2993],mask=back_pred)
                        write_image_tensor(depth_pred, output_depth_pred, 'gauss', mean=[0.3086], std=[0.3101],mask=back_pred)
                        write_image_tensor(normal_pred, output_normal_pred, 'gauss', mean=[0.5623, 0.2883, 0.2918],
                                           std=[0.4808, 0.2616, 0.2623],mask=back_pred)
                        write_image_np(bw_pred, output_bw_pred)
                        """gt"""
                        write_image_tensor(ori_gt, output_ori, 'std',mask=mask_gt)
                        write_image_tensor(uv_gt, output_uv_gt, 'std', device=device,mask=mask_gt)
                        ...
                        write_image_tensor(cmap_gt, output_cmap_gt, 'gauss', mean=[0.1109, 0.3162, 0.2862],
                                           std=[0.1101, 0.3288, 0.2993],mask=mask_gt)
                        write_image_tensor(depth_gt, output_depth_gt, 'gauss', mean=[0.3086], std=[0.3101],mask=mask_gt)
                        write_image_tensor(normal_gt, output_normal_gt, 'gauss', mean=[0.5623, 0.2883, 0.2918],
                                           std=[0.4808, 0.2616, 0.2623],mask=mask_gt)
    ...
    if args.write_txt:
        ...
        f.write(
            'Epoch: {} \t Test Loss: {:.6f}, \t ab: {:.4f}, \t cmap: {:.4f}, \t uv: {:.6f}, \t normal: {:.4f}, \t depth: {:.4f}, \t back: {:.6f} , \t constrain 3d to normal: {:.4f}, \t constrain 3d to depth: {:.4f}, \t constrain normal to depth: {:.4f}, \t constrain depth to normal: {:.4f}, \t loss tv: {:.6f}, CC_uv: {}\t CC_cmap: {}\t CC_ab: {}\t CC_bw: {}\t CC_dewarp_ori: {}\t CC_dewarp_ab: {}\n'.format(
                ...
                test_cons_n2d.item(), test_cons_d2n.item(), test_loss_tv.item(), cc_uv, cc_cmap, cc_ab, cc_bw, cc_dewarp_ori,
                cc_dewarp_ab))
        f.close()
    if args.write_summary:
        print('sstep', sstep)
